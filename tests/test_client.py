...
def test_repr(session):
    iss = client.ISSClient(session, "test_url", {"a": "b"})
...
def test_get_wrong_json(monkeypatch, session):
    ...
    monkeypatch.setattr(requests.Response, "json", lambda _: [0, 1, 2])
    with pytest.raises(client.ISSMoexError) as error:
        iss.get()
...
def test_make_query_not_empty(session):
    iss = client.ISSClient(session, "test_url", {"test_param": "test_value"})
    # noinspection PyProtectedMember
    query = iss._make_query()
    assert isinstance(query, dict)
    assert len(query) == 3
...
def test_wrong_cursor_size(monkeypatch, session):
    ...
    monkeypatch.setattr(iss, "get", lambda _: fake_cursor)
    with pytest.raises(client.ISSMoexError) as error:
        iss.get_all()
    assert "Некорректные данные history.cursor [0, 1] для начальной позиции 0" in str(error.value)
